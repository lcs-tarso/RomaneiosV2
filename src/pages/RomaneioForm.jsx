...
export default function RomaneioForm() {
  ...
  const [romaneio, setRomaneio] = useState({ numeroControle: "" });
  ...
  const salvarRomaneio = (e) => {
    ...
    const nomeArquivo = `${numero}_Romaneio de entrega de notas_${dataStr}`;
        ...
        {notas.map((nota, index) => (
          <div key={index} className="border p-4 rounded-md space-y-2 bg-gray-50">
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <input placeholder="Centro de Custo" value={nota.centroCusto} onChange={e => atualizarNota(index, "centroCusto", e.target.value)} />
